...
def main() -> None:
    ...
    include_backlog_carryover = st.checkbox(
        "Include PRD backlog items & ticket links under each epic/story",
        value=True,
        help="Adds **Backlog carry-over (from PRD)** to every story and epic-level ticket blocks when the PRD lists Jira/Azure DevOps/GitHub/Linear links or backlog rows — verbatim IDs/URLs so nothing is missed.",
        key="include_backlog_carryover",
    )
...
(Source: {source})"""
        ...
        if include_backlog_carryover:
            user_msg += """

---

**Output request (backlog):** For every epic and story, follow the prompt rules for **Epic backlog / tickets (from PRD)** (epic-level only when needed) and **Backlog carry-over (from PRD)** (every story). Copy ticket keys and URLs **exactly** as they appear in the PRD above. Use **None** where the PRD has no item for that story."""
        else:
            user_msg += """

---

**Output request (backlog):** Do **not** include the sections **Epic backlog / tickets (from PRD)** or **Backlog carry-over (from PRD)** anywhere. Omit those headings entirely."""
