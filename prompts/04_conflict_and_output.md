...
- Output epics in logical dependency order — foundational flows first,
  advanced flows last.
- Each epic: one-line user goal, then optionally **Epic assumptions** (short bullet list, only to remove repetition — see REDUNDANCY PREVENTION), then optionally **Epic backlog / tickets (from PRD)** when the PRD has epic-wide or cross-story tickets (see BACKLOG & TICKET LINKS), then stories.
...
- Complete every field for every story. If the user message enables backlog mapping, include **Backlog carry-over (from PRD)** on each story; if the user message disables it, omit those headings and treat the story template as if that field were not required. **Concise does not mean incomplete:** keep every **testable** PRD requirement, but express it with **third person**, **plain language**, and **grouped** criteria per STORY TEMPLATE (avoid walls of near-duplicate lines).
