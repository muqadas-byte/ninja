...
- Ticketanos rejects as too vague: "Bug fix", "Issue", "UI problem",
  "Fix: issue on the loading screen", "Proposal not opening", "Snippets improvements",
  "Funder Notes - show devider" — and any title that does not communicate what changes
...
### Bug or fix stories (when the PRD describes a defect or regression)

When the PRD item is a **bug/fix** rather than a new capability:

- Title pattern: **[Area/screen] — [specific symptom]** (e.g. "Proposals list — row click
  does not open proposal detail").
- Do **not** use generic bug titles (see UNACCEPTABLE above).
- The story must still state **current behavior**, **expected behavior**, and **where** it
  occurs (see STORY TEMPLATE). Put repro steps in **Acceptance Criteria** when the PRD
  provides them.
...
- Never describe problems with vague phrases alone — **"not working properly"**, **"broken"**,
  **"weird behavior"** — without an example, repro steps, or expected vs actual behavior.
- One story = one coherent functional change. If the PRD bundles multiple distinct flows,
  **split** into separate stories (Ticketanos sends bundled tickets back to Uncooked).
